    ...
    if request.method == 'POST':
        ...
        for w, t in zip(weekdays, time_slots):
            ...
            if instructor_course_time:
                return render_template('write_course_info.html', warning='該授課教師此時段已有課程，請重新輸入', course_code=course_code)

        # 檢查開課班級是否存在
        for class_name in class_names:
            is_class_exist = query_db('''SELECT * FROM Class WHERE class_name=?''', [class_name])
            if not is_class_exist:
                return render_template('write_course_info.html', warning='該開課班級不存在，請重新輸入', course_code=course_code)
        ...
        for weekday, time_slot in zip(weekdays, time_slots):
            ...
            for sl in same_location:
                for st in same_time:
                    if sl == st:
                        return render_template('write_course_info.html', warning='該上課地點此時段已有課程，請重新輸入', course_code=course_code)
                
        # 檢查班級限制的班級是否存在
        for class_limit in class_limits:
            is_class_exist = query_db('''SELECT * FROM Class WHERE class_name=?''', [class_limit])
            if not is_class_exist:
                return render_template('write_course_info.html', warning='班級限制的班級不存在，請重新輸入', course_code=course_code)
        ...
        if int(max_students) > location_max_students[0][0]:
            return render_template('write_course_info.html', warning='該上課地點無法容納此課程的最大修課人數，請重新輸入', course_code=course_code)
        ...
        return render_template('write_course_info.html', success='已成功紀錄課程資訊', course_code=course_code)

    return render_template('write_course_info.html', course_code=course_code)

@app.route('/course_query/check_course_query_input', methods=['POST'])
