...
def write_course_course_info():
    ...
    for w, t in zip(weekdays, time_slots):
        ...
        if instructor_course_time:
            return jsonify({'status': 'error', 'message': '該授課教師此時段已有課程，請重新輸入'})

    # 檢查開課班級是否存在
    for class_name in class_names:
        is_class_exist = query_db('''SELECT * FROM Class WHERE class_name=?''', [class_name])
        if not is_class_exist:
            return jsonify({'status': 'error', 'message': '該開課班級不存在，請重新輸入'})
    ...
    for weekday, time_slot in zip(weekdays, time_slots):
        ...
        for sl in same_location:
            for st in same_time:
                if sl == st:
                    return jsonify({'status': 'error', 'message': '該上課地點此時段已有課程，請重新輸入'})
    ...
    for class_limit in class_limits:
        if class_limit=="all":
            ...
            class_limits=[]
            for i in is_class_exist:
                class_limits.append(i[0])           
            break
        else:
